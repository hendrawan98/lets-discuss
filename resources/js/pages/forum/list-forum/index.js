            ...
            <CardContainer>
              <div style={{float: 'left', width: '35em', borderRight: '1px solid black'}}>
                <div style={{marginRight: '20px'}}>
                  ...
                  <Paragraph size={20}>{val.forumContent}</Paragraph>
                </div>
              </div>
              <div style={{float: 'right', width: '14.9em'}}>
                <div style={{ marginLeft: '20px', height: '10em', display: 'flex', alignItems: 'center' }}>
                  <Button onClick={() => window.location.href = `http://letsdiscuss.loc/view-forum/${val.forumTitle.replace(' ', '-')}`}>Baca</Button><br />
                  <CopyToClipboard text={`http://letsdiscuss.loc/view-forum/${val.forumTitle.replace(' ', '-')}`} onCopy={() => setCopied(true)} style={{margin: "0 1.5em"}}>
    ...
    return (
      <CardContainer style={{ width: '52.5em' }}>
        <div style={{ float: 'left', width: '13em', height: '2.3em', display: 'flex', alignItems: 'center' }}>
          <label style={{marginRight: '0.5em'}}>Urutkan:</label>
